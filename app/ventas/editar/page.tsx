...
function EditarVentaContent() {
  ...
  useEffect(() => {
    ...
    if (user === null || (user.role !== "supervisor" && user.role !== "admin")) {
      setDenied(true);
      setTimeout(() => {
  ...
  useEffect(() => {
    async function fetchVenta() {
      if (ventaId) {
        ...
        setTotal(Number(data.inv_total || 0));
        setLoading(false);
      }
    }
    fetchVenta();
  ...
  }, [productos, venta?.inv_descuento, venta?.inv_desc_extra]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0a0a0a] via-[#18181b] to-[#0a0a0a]">
        <span className="text-red-400 text-xl font-bold">
          Acceso denegado: solo supervisores y administradores pueden editar ventas.
        </span>
      </div>
    );
  }
      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-8 flex-1">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {/* Producto */}
            <section className="bg-[#0a0a0a] rounded-xl p-6 flex flex-col gap-4 shadow-lg">
              <h3 className="font-semibold text-purple-400 mb-2 text-lg">Productos</h3>
                    ...
                    <tbody>
                      {productos.map((prod, idx) => (
                        <tr key={idx} className="border-t border-blue-900">
                          ...
                          <td className="p-2">${(prod.precio * prod.cantidad).toFixed(2)}</td>
                          <td className="p-2">
                            <button
              ...
              <div className="flex flex-col gap-2 mt-4">
                ...
                <input
                  ...
                  onChange={handleChange}
                />
                <label className="font-medium text-blue-200">Descuento Extra</label>
                <input
                  type="number"
                  name="inv_desc_extra"
                  min={0}
            ...
            <section className="bg-[#0a0a0a] rounded-xl p-6 flex flex-col gap-4 shadow-lg">
              ...
              <div className="mt-4 p-4 bg-[#232336] rounded">
                ...
                <p className="text-lg font-bold">
                  <span className="font-medium">Total:</span> ${Number(total).toFixed(2)}
                </p>
              </div>
              <button
                type="submit"
